						...
						execute: async (state, argument, _) => {
							...
							if (command && module)
								...
								if (commands.length > 0)
									console.log(
										`${name} ${module.name} <command>\t\tRun command\n${name} help ${
											module.name
										} <command>\tGet help on command\n\nCommands:\n${commands
											.map(c => `${c?.name.padEnd(10, " ")}\t${c?.description}`)
											.join("\n")}\n\nFlags:\n${flags
											.map(f => `--${f?.long} ${f?.usage.padEnd(20, " ")}\t${f?.description}`)
											.join("\n")}`
									)
								else
